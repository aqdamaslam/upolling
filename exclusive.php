...
$sql="SELECT SUM(yes) FROM exclusive";
$records=mysql_query($sql);

$sql="SELECT SUM(no) FROM exclusive";
$records1=mysql_query($sql);

$sql="SELECT SUM(may_be) FROM exclusive";
$records2=mysql_query($sql);

$sql="SELECT SUM(not_interested) FROM exclusive";
...
<!-- Favicon -->
  ...
  <meta name="description" content="Janta Ka Rujhaan,">
  <meta name="keywords" content="Rape Law">
  <meta name="author" content="Penalty">
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      ...
      <li class="active"><a href="exclusive.php">Exclusive Poll</a></li>
...
<div class="navbar" align="center">
  <img src="img/exclusive/exclusive.png" alt="Sat" class="responsive" style="width:70px"></br>
</div> 
<h3>Is Death Penalty Law Will Help To Reduce Rape Case ?</h3>
...
<form class="form-horizontal" name="form1" action="exclusive_in.php" onsubmit="return validate()" method="post"> 
...
<input type="hidden" name="ip" id="ip" style="border: none; border-color: white; background-color:white; color:white;"  value="<?php echo $_SERVER['REMOTE_ADDR'];   ?>">
		...
		<label class="container">	
		<label class="col-md-3 box"><h4><b>Yes = <?php while($poll=mysql_fetch_assoc($records)){ echo $poll['SUM(yes)']; } ?></b></h4><label class="btn btn-success"><img src="img/exclusive/yes.PNG" alt="..." class="img-thumbnail img-check"><input type="radio" name="yes" id="yes" value="1" class="hidden" autocomplete="off"></label></label>
		<label class="col-md-3 box"><h4><b>No = <?php while($poll=mysql_fetch_assoc($records1)){ echo $poll['SUM(no)']; } ?></b></h4><label class="btn btn-warning"><img src="img/exclusive/no.PNG" alt="..." class="img-thumbnail img-check"><input type="radio" name="no" id="no" value="1" class="hidden" autocomplete="off"></label></label>
		<label class="col-md-3 box"><h4><b>May Be = <?php while($poll=mysql_fetch_assoc($records2)){ echo $poll['SUM(may_be)']; } ?></b></h4><label class="btn btn-info"><img src="img/exclusive/maybe.PNG" alt="..." class="img-thumbnail img-check"><input type="radio" name="may_be" id="may_be" value="1" class="hidden" autocomplete="off"></label></label>
		<label class="col-md-3 box"><h4><b>Not Interested = <?php while($poll=mysql_fetch_assoc($records3)){ echo $poll['SUM(not_interested)']; } ?></b></h4><label class="btn btn-danger"><img src="img/exclusive/not_interested.PNG" alt="..." class="img-thumbnail img-check"><input type="radio" name="not_interested" id="not_interested" value="1" class="hidden" autocomplete="off"></label></label>
		</label>
